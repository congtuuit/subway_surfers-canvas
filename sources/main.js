...
`;
  ...
  __police = new Police(gl, [-6, -4, 0]);
  __police.speedz = player_speed;
  ...
  for (var i = 0; i < 50; i++) {
    ...
    y = -4;
    if (i == 0) z = -30;
    else z = coins[coins.length - 1].pos[2] - (Math.random() * 30 - 15);
  ...
  for (var i = 0; i < 10; i++) {
    ...
    var train_speed;
    var j = Math.floor(Math.random() * 3);
    if (j == 0) train_speed = 0.2;
    else if (j == 1) train_speed = 0.5;
    else train_speed = 0.7;
  ...
  function render(now) {
    ...
    d = new Date();
    if (
      d.getTime() * 0.001 - policeCaughtUp >= 5 &&
      d.getTime() * 0.001 - policeCaughtUp <= 10
    )
      __police.speedz = player_speed / 2;
    else __police.speedz = player_speed;
    __police.pos[2] -= __police.speedz;

    if (player.pos[0] > 6) player.pos[0] = 6;
    if (player.pos[0] < -6) player.pos[0] = -6;
    __police.pos[0] = player.pos[0];
    __police.pos[1] = player.pos[1];
          ...
          for (var i = 0; i < n; i++) {
            if (player.pos[0] == trainF[i].pos[0]) {
              if (
                ...
                player.pos[1] >= trainT[i].pos[1]
              ) {
                if (
                  player.pos[2] <= trainF[i].pos[2] &&
                  player.pos[2] >= trainF[i].pos[2] - 20
                ) {
                  player.pos[1] = trainT[i].pos[1] + 1;
                  break;
                }
          ...
          for (var i = 0; i < n; i++) {
            if (player.pos[0] == boxes[i].pos[0]) {
              if (player.pos[1] <= boxes[i].pos[1] + 3.5) {
                if (
                  player.pos[2] <= boxes[i].pos[2] + 3.5 &&
                  player.pos[2] >= boxes[i].pos[2] - 3.5
                ) {
    ...
    for (var i = 0; i < num_coins; i++) {
      if (coins[i].exist == true) {
        if (coins[i].pos[0] == player.pos[0]) {
          if (
            coins[i].pos[1] >= player.pos[1] - 0.75 &&
            coins[i].pos[1] <= player.pos[1] + 0.75
          ) {
            if (
              coins[i].pos[2] >= player.pos[2] - 0.5 &&
              coins[i].pos[2] <= player.pos[2] + 0.5
            ) {
              coins[i].exist = false;
              coins_collected += 1;
    ...
    if (!player.hoverboard) {
      ...
      for (var i = 0; i < num_trains; i++) {
        if (player.pos[0] == trainF[i].pos[0]) {
          if (
            player.pos[1] >= trainF[i].pos[1] - 4 &&
            player.pos[1] <= trainF[i].pos[1] + 4
          ) {
            if (
              player.pos[2] >= trainF[i].pos[2] - 18 &&
              player.pos[2] <= trainF[i].pos[2]
            ) {
      ...
      for (var i = 0; i < num_boxes; i++) {
        if (player.pos[0] == boxes[i].pos[0]) {
          if (
            player.pos[1] >= boxes[i].pos[1] - 2 &&
            player.pos[1] <= boxes[i].pos[1] + 2
          ) {
            if (
              player.pos[2] <= boxes[i].pos[2] + 3 &&
              player.pos[2] >= boxes[i].pos[2] - 3
            ) {
              score = -player.pos[2] + coins_collected;
              Die();
      ...
      for (var i = 0; i < num_manholes; i++) {
        if (player.pos[0] == manholes[i].pos[0]) {
          if (player.pos[1] <= -4) {
            if (
              player.pos[2] <= manholes[i].pos[2] + 2.3 &&
              player.pos[2] >= manholes[i].pos[2] - 2.3
            ) {
              score = -player.pos[2] + coins_collected;
              Die();
      ...
      for (var i = 0; i < num_high; i++) {
        if (i != obstacle_hit) {
          if (player.pos[0] == duck_obs_stop[i].pos[0]) {
            if (
              player.pos[1] >= duck_obs_stop[i].pos[1] - 4 &&
              player.pos[1] <= duck_obs_stop[i].pos[1] + 4
            ) {
              if (
                duck_obs_stop[i].pos[2] >= player.pos[2] - 0.7 &&
                duck_obs_stop[i].pos[2] <= player.pos[2] + 0.7
              ) {
                d = new Date();
      ...
      for (var i = 0; i < num_low; i++) {
        if (i != obstacle_hit) {
          if (player.pos[0] == jump_obs[i].pos[0]) {
            if (
              player.pos[1] >= jump_obs[i].pos[1] - 2 &&
              player.pos[1] <= jump_obs[i].pos[1] + 2
            ) {
              if (
                player.pos[2] <= jump_obs[i].pos[2] + 0.2 &&
                player.pos[2] >= jump_obs[i].pos[2] - 0.2
              ) {
                d = new Date();
                if (d.getTime() * 0.001 - policeCaughtUp <= 10) {
      ...
      for (var i = 0; i < num_rope; i++) {
        if (i != obstacle_hit) {
          if (
            rope_stop[i].pos[1] <= player.pos[1] + 0.5 &&
            rope_stop[i].pos[1] >= player.pos[1] - 0.5
          ) {
            if (
              player.pos[2] <= rope_stop[i].pos[2] + 0.02 &&
              player.pos[2] >= rope_stop[i].pos[2] - 0.02
            ) {
              d = new Date();
              if (d.getTime() * 0.001 - policeCaughtUp <= 10) {
    ...
    for (var i = 0; i < num_trains; i++) {
      for (var j = 0; j < num_boxes; j++) {
        if (boxes[j].pos[0] == trainF[i].pos[0]) {
          if (
            boxes[j].pos[2] - 5 <= trainF[i].pos[2] &&
            boxes[j].pos[2] - 4.5 >= trainF[i].pos[2]
          ) {
            train_speeds[i] = 0;
          }
        }
    ...
    for (var i = 0; i < num_trains; i++) {
      for (var j = 0; j < num_high; j++) {
        if (trainF[i].pos[0] == duck_obs_stop[j].pos[0]) {
          if (
            trainF[i].pos[2] >= duck_obs_stop[j].pos[2] - 5 &&
            trainF[i].pos[2] <= duck_obs_stop[j].pos[2] - 4.5
          ) {
            train_speeds[i] = 0;
          }
        }
    ...
    for (var i = 0; i < num_trains; i++) {
      for (var j = 0; j < num_low; j++) {
        if (trainF[i].pos[0] == jump_obs[j].pos[0]) {
          if (
            trainF[i].pos[2] >= jump_obs[j].pos[2] - 5 &&
            trainF[i].pos[2] <= jump_obs[j].pos[2] - 4.5
          ) {
            train_speeds[i] = 0;
          }
        }
    ...
    for (var i = 0; i < num_boots; i++) {
      if (boots[i].exist) {
        ...
        if (player.pos[0] == boots[i].pos[0]) {
          if (
            player.pos[1] >= boots[i].pos[1] - 1.2 &&
            player.pos[1] <= boots[i].pos[1] + 1.2
          ) {
            if (
              player.pos[2] >= boots[i].pos[2] - 1.2 &&
              player.pos[2] <= boots[i].pos[2] + 1.2
            ) {
              boots[i].exist = false;
    ...
    for (var i = 0; i < num_fb; i++) {
      if (flying_boost[i].exist) {
        ...
        if (player.pos[0] == flying_boost[i].pos[0]) {
          if (
            player.pos[1] >= flying_boost[i].pos[1] - 1.75 &&
            player.pos[1] <= flying_boost[i].pos[1] + 1.75
          ) {
            if (
              player.pos[2] >= flying_boost[i].pos[2] - 1.75 &&
              player.pos[2] <= flying_boost[i].pos[2] + 1.75
            ) {
    ...
    for (var i = 0; i < 2; i++) {
      ...
      if (hoverboard[i].exist) {
        if (player.pos[0] == hoverboard[i].pos[0]) {
          if (
            player.pos[1] >= hoverboard[i].pos[1] - 1.2 &&
            player.pos[1] <= hoverboard[i].pos[1] + 1.2
          ) {
            if (
              player.pos[2] >= hoverboard[i].pos[2] - 1.2 &&
              player.pos[2] <= hoverboard[i].pos[2] + 1.2
            ) {
...
function drawScene(gl, programInfo, deltaTime) {
  if (theme_flag == 1) {
    if (theme == 1) {
      ...
      if (greyScale) gl.clearColor(50 / 255, 50 / 255, 50 / 255, 1.0);
    }
    if (theme == 2) {
  ...
  //__police.drawCube(gl, viewProjectionMatrix, programInfo, deltaTime);
  ...
  for (var i = 0; i < num_high; i++) {
    ...
    duck_obs_stand1[i].drawCube(
      gl,
      viewProjectionMatrix,
      programInfo,
      deltaTime
    );
    duck_obs_stand2[i].drawCube(
      gl,
      viewProjectionMatrix,
      programInfo,
      deltaTime
